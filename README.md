...
└────┬────┘ └────────┬─────────┘
     └───────┬───────┘
             ▼
  ┌──────────────────────┐
  │  Legal Analysis Agent│  ← 고소 가능 여부 + 예상 처벌 판단
  └──────────┬───────────┘
             ▼
  ┌──────────────────────┐
  │ Report Generation    │  ← 비전문가 언어로 결과 변환
  │      Agent           │
  └──────────┬───────────┘
             ▼
       결과 화면 출력
...
| **Legal Analysis Agent** | 법령 + 판례 종합 → 고소 가능성, 예상 형량, 증거 수집 안내 |
| **Report Agent** | 결과를 일반인 언어로 변환, 위험도 레벨 산출 |
