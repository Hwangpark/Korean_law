...
function ProbabilityPill({ prob }: { prob: 'high' | 'medium' | 'low' }) {
  const map = { high: ['성립 가능성 높음', 'prob-high'], medium: ['성립 가능성 보통', 'prob-medium'], low: ['성립 가능성 낮음', 'prob-low'] };
...
export default function App() {
  ...
  async function handleAnalyze() {
    ...
    // Simulate step-by-step progress while fetching
    const stepDelay = 600;
    AGENT_STEPS.forEach((step, i) => {
      setTimeout(() => {
        setAgentProgress((prev) => [...prev, step.id]);
      }, i * stepDelay);
    });
    ...
    try {
      const res = await fetch(`${API_BASE}/api/analyze`, {
        ...
        body: JSON.stringify({ text: text.trim(), context_type: contextType }),
      });
      ...
      await new Promise((r) => setTimeout(r, AGENT_STEPS.length * stepDelay + 400));
      setResult(data.legal_analysis);
      setView('results');
    } catch (err) {
        ...
        <main className="input-main">
          <section className="hero">
            ...
            <h1 className="hero-title">
              온라인 피해, 법적으로<br />따져드립니다
            </h1>
            <p className="hero-desc">
              커뮤니티 게시글, 게임 채팅, 메신저 대화를 붙여넣으면<br />
          ...
          <div className="input-card">
            ...
            <p className="input-disclaimer">
              업로드된 내용은 분석 후 즉시 삭제되며 서버에 저장되지 않습니다.
              본 서비스는 법률 정보 제공 목적이며 법적 효력이 없습니다.
            </p>
          </div>
          ...
          <div className="analyzing-card">
            ...
            <div className="pipeline">
              {AGENT_STEPS.map((step, i) => {
                ...
                return (
                  <div key={step.id} className={`pipeline-step ${done ? 'step-done' : active ? 'step-active' : 'step-waiting'}`}>
                  ...
                  {result.precedent_cards.map((p, i) => (
                    <div key={i} className="precedent-card">
                      <div className="precedent-header">
                        ...
                        <span className={`verdict-badge ${p.verdict === '유죄' ? 'verdict-guilty' : 'verdict-not-guilty'}`}>
                          {p.verdict}
                        </span>
                      </div>
