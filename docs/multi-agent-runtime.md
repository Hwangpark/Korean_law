...
`raw input -> ocr/text normalization -> classifier(signal detection + guided extraction) -> Scope Filter -> Retrieval Planner -> law/precedent retrieval -> Evidence Rerank -> Evidence Pack -> grounded analysis`
...
- `analysis`는 evidence pack을 기준으로 grounded output을 만든다.
...
sequenceDiagram
    ...
    A-->>O: grounded legal analysis + citation map
...
- evidence pack 중심 grounded analysis를 수행한다.
...
**runtime stage 이름은 늘리지 않되, 내부 logical substep을 명시하고 evidence pack 중심 grounded analysis를 수행한다.**
